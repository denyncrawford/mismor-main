<template>
  <div v-show="loading" class="top-0 absolute flex items-center justify-center w-screen h-screen z-50 bg-white">
    <div class="flex flex-col items-center text-center justify-center">
      ...
      <h1 class="px-2 py-1 text-white bg-black text-xs mt-10">Cargando</h1>
    </div>
  </div>
</template>
...
export default {
  ...
  watch: {
    loading(newS, old) {
      newS ? document.body.classList.add('scrollbar-hidden') : document.body.classList.remove('scrollbar-hidden')
    }
  },
  mounted() {
